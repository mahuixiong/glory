		...
		<div class="cus-content">
			...
			<div class="pageing-con">
				<ul class="pageing-info">
					<li class="pageing-item head"><a href="selectAllUser?pageNum=1&username=${username}&company=${company}&checkStatus=${checkStatus}" title="">首页</a></li>
					<li class="pageing-item prev">
						<c:if test="${jsonmap.user.hasPreviousPage}">
							<a href="selectAllUser?pageNum=${jsonmap.user.pageNum-1}&username=${username}&company=${company}&checkStatus=${checkStatus}" title="">上一页</a>
						</c:if>
					...
					<c:forEach items="${jsonmap.user.navigatepageNums}" var="page_num">
						...
						<c:if test="${page_num !=jsonmap.user.pageNum}">
							<li class="pageing-item  disabled active">
								<a href="selectAllUser?pageNum=${page_num}&username=${username}&company=${company}&checkStatus=${checkStatus}" title="">${page_num}</a>
							</li>
						</c:if>
					</c:forEach>
					<li class="pageing-item next">
						<c:if test="${jsonmap.user.hasNextPage}">
							<a href="selectAllUser?pageNum=${jsonmap.user.pageNum+1}&username=${username}&company=${company}&checkStatus=${checkStatus}" title="">下一页</a>
						</c:if>
						<c:if test="${jsonmap.user.hasNextPage==false}">
							<a href="javascript:void(0)" title="">下一页</a>
						</c:if>
					</li>
					<li class="pageing-item foot"><a href="selectAllUser?pageNum=${jsonmap.user.pages}&username=${username}&company=${company}&checkStatus=${checkStatus}" title="">尾页</a></li>
				</ul>
			</div>
		    <div class="page">
			</div>
	...
	<script>
		...
		function findUser() {
			...
			<%--$.ajax({--%>
				...
				<%--{--%>
				    location.href="<%=basePath%>rest/user/selectAllUser?username="+username+"&company="+company+"&checkStatus="+checkStatus
//				}
//			})
        }
